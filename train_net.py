    ...
    def build_train_loader(cls, cfg):
        ...
        mapper = DatasetMapper(cfg, is_train=True, augmentations=\
                                    [T.ResizeShortestEdge(short_edge_length=(640, 672, 704, 736, 768, 800), max_size=1333, sample_style='choice'),\
                                        #T.RandomApply(T.RandomCrop('relative_range', (0.4, 0.6))),\
                                            T.RandomApply(T.RandomLighting(0.7),prob=0.4),\
                                                T.RandomApply(T.RandomSaturation(intensity_min=0.4, intensity_max=1.25),  prob=0.40),\
                                                    T.RandomApply(T.RandomContrast(intensity_min=0.76, intensity_max=1.25), prob=0.30),\
                                                        T.RandomApply(T.RandomBrightness(intensity_min=0.75, intensity_max=1.25),prob=0.40)])
        if cfg.SEED!=-1:
